...
public class ApplicationHttpUtils {
	...
	public String post(final String context, final String body) {
		try {
			HttpResponse response = Request.Post(baseUrl + context).bodyString(body, ContentType.APPLICATION_JSON)
	...
	public String get(final String context) {
		try {
			HttpResponse response = Request.Get(baseUrl + context).execute().returnResponse();
			assertIs200(response);
			return EntityUtils.toString(response.getEntity());
		} catch (IOException e) {
			throw new RuntimeException(e);
